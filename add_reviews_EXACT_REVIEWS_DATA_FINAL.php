 ...
 * ============================================================
 *
 * Reads the Brand from <span> and the complete phone name from
 * <h3> inside the SAME phone-card, then builds the image URL.
 * Only the image src is changed.
 */

function normalizeCardImagePart(string $value): string
{
...
function fixPhoneCardAlternativeImages(string $html): string
{
    return preg_replace_callback(
        ...
        static function (array $match): string {
            ...
            if (!preg_match('~<span\b[^>]*>(.*?)</span>~isu', $card, $brandMatch)) {
                return $card;
            }

            if (!preg_match('~<h3\b[^>]*>(.*?)</h3>~isu', $card, $titleMatch)) {
                return $card;
            }

            $brand = normalizeCardImagePart($brandMatch[1]);
            $title = normalizeCardImagePart($titleMatch[1]);
            ...
            return preg_replace_callback(
                ...
                static function (array $imageMatch) use ($imagePath): string {
                    $src = $imageMatch[3];

                    if (
                        stripos($src, '_alt1_3_2.webp') === false &&
                        stripos($src, '_alt2_3_2.webp') === false &&
                        stripos($src, '_alt3_3_2.webp') === false
                    ) {
                        return $imageMatch[0];
                    }
 ...
 * ============================================================
 *
 * Updates reviews-data.php after the review HTML file is created.
 * The generated review URL is always:
 * reviews/{slug}-reviews.html
 *
 * This function does not modify the HTML template, href values,
 * menu, footer, CSS, JavaScript, or Quick Fill.
 */
...
function updateReviewsData(
    ...
    /*
     * READ EXISTING reviews-data.php.
     * If the old file cannot be decoded, start with an empty list
     * instead of blocking creation of the new review.
     */
    if (is_file($dataPath)) {
        ...
        if ($rawFile !== false && trim($rawFile) !== '') {
            ...
            if (
                ...
                /* PHP returning an array or outputting JSON */
                $existingReturn = null;
                $existingOutput = '';
                $bufferStarted = false;
                ...
                if (
                    ...
                    $decodedOutput = json_decode(trim($existingOutput), true);
    ...
    /*
     * ADD / REPLACE REVIEW.
     * The URL is the unique identifier, so submitting the same
     * review again updates it instead of creating a duplicate.
     */
    $newReview = [
        'brand' => trim($brand),
        'title' => trim($title),
    ...
    foreach ($reviews as $index => $review) {
        if (
            is_array($review) &&
            trim((string)($review['url'] ?? '')) === trim($url)
        ) {
    ...
    foreach ($reviews as $review) {
        ...
        $reviewBrand = trim((string)($review['brand'] ?? ''));
    ...
    /*
     * One file, two modes:
     * - include() => returns PHP array
     * - direct request => returns JSON
     */
    ...
    /*
     * Write directly to the same reviews-data.php path.
     * Do not change its filename or location.
     */
...
if (
    ...
    foreach ($allPlaceholders as $index => $field) {
        ...
        if (!in_array($upperField, $imageFields, true)) {
            continue;
        }
        ...
        if (
            ...
            $generatedPath = findActualImagePath(
                '',
                $imageBrand,
                $imagePhoneTitle,
                $upperField
        ...
        $data[$fieldNumber] = $generatedPath;
        $generatedImages[$upperField] = $generatedPath;
    }
     ...
     * ============================================================
     *
     * The template is the sole source of HTML.
     * Nothing is rewritten, normalized, repaired, or rebuilt.
     *
     * Only {{PLACEHOLDER}} values are replaced.
     * All literal href/src/nav/footer/CSS/JS/text remains exactly
     * as it exists in the template.
     */
     ...
     * ============================================================
     * Read <span> and <h3> from each phone-card itself.
     * Only alternative image src attributes are changed.
     */
    ...
    /*
     * No href fixer.
     * No menu/footer rewriting.
     */

    /*
     * IMAGE PATHS ARE GENERATED SERVER-SIDE.
     * They are not inserted into the template except through their
     * corresponding image placeholders above.
     */

    foreach ($generatedImages as $field => $path) {
        error_log(
            '[TDH IMAGE] ' . $field . ': ' . $path
        );
    }
