...
function fixEmptyImageSources(
    ...
    /*
     * ONLY these 9 image containers are touched.
     * No href attribute is read or modified.
     */
    $containerMap = [
    ...
    foreach ($containerMap as $className => $field) {
        ...
        /* Find the img inside ONLY the named container and replace src. */
        $pattern = '~(<(?:div|section|figure|article|span)\\b[^>]*\\bclass\\s*=\\s*)(["\\\'])([^"\\\']*\\b' .
            preg_quote($className, '~') .
            '\\b[^"\\\']*)\\2([^>]*>\\s*<img\\b[^>]*\\bsrc\\s*=\\s*)(["\\\'])[^"\\\']*\\5~isu';
