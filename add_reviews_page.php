...
function getFirstReviewFieldValue(
    ...
    foreach ($fieldNames as $fieldName) {
        $value = getFieldValue($allPlaceholders, $data, $fieldName);
        if ($value !== '') {
            return $value;
        }
    }
...
function updateReviewsData(
    ...
    if (is_file($dataPath)) {
        try {
            $existing = include $dataPath;
        } catch (Throwable $e) {
            return false;
        }
        if (!is_array($existing) || !isset($existing['reviews']) || !is_array($existing['reviews'])) {
            return false;
        }
    ...
    $reviews[] = [
        ...
        'category' => trim($category) !== '' ? trim($category) : 'Smartphones',
        'image' => trim($image),
        'url' => trim($url),
    ];
    ...
    foreach ($reviews as $review) {
        ...
        if ($reviewBrand !== '' && !in_array($reviewBrand, $brands, true)) {
            $brands[] = $reviewBrand;
        }
    }
    ...
    if (file_put_contents($temporaryPath, $php, LOCK_EX) === false) {
        return false;
    }
...
if (
    ...
    if (
        ...
        $score = getFirstReviewFieldValue(
            $allPlaceholders,
            $data,
            ['SCORE', 'RATING']
        );

        $description = getFirstReviewFieldValue(
            $allPlaceholders,
            $data,
            ['DESCRIPTION', 'SUMMARY', 'REVIEW_SUMMARY', 'VERDICT']
        );

        $category = getFirstReviewFieldValue(
            $allPlaceholders,
            $data,
            ['CATEGORY', 'PRODUCT_CATEGORY']
        );

        $heroImage = $generatedImages['HERO_IMAGE'] ?? '';
        $reviewUrl = 'reviews/' . $outputName;

        if (!updateReviewsData(
            $brand,
            $title,
            $score,
            $description,
